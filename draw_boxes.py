...
if __name__ == '__main__':
    franka = FrankaRobot()
    franka.set_base_offset([-1, 0, 0])

    init_joint = franka.INIT_JOINTS
    target_joint = np.array([0.0, 5e-1, 0.0, -2.3, 0.0, 2.8, 7.8e-1])
    ...
    for i, franka_box_pose in enumerate(franka_box_poses):
        franka_box_pos = franka_box_pose[:3, 3]

        franka_box_axes = franka_box_pose[:3, :3]

        franka_box_vertex_offsets = franka._collision_box_vertices_offset[i]
        franka_box_vertices = franka_box_vertex_offsets.dot(franka_box_axes.T) + franka_box_pos

        ax.scatter(franka_box_vertices[:, 0], franka_box_vertices[:, 1], franka_box_vertices[:, 2])
        draw_box(ax, franka_box_vertices)
    ...
    for i in range(len(tree_XYZ)):
        ...
        tree_boxes_vertices.append(box_vertices)

    for i, tree_box_vertices in enumerate(tree_boxes_vertices):
        ax.scatter(tree_box_vertices[:, 0], tree_box_vertices[:, 1], tree_box_vertices[:, 2])
        draw_box(ax, tree_box_vertices)
