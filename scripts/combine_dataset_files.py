...
for tree in range(TREE_START, TREE_NUM):
    ...
    coeff_arrays.append(np.load('X_coeff_stiff_damp_tree%s.npy'%(tree)))
    edge_def_arrays.append(np.load('X_edge_def_tree%s.npy'%(tree)))
    for env in range(0, ENV_NUM):
        ...
        x_vert_arrays.append(np.load('X_vertex_init_pose_tree%s_env%s.npy'%(tree, env)))
        force_applied_arrays.append(np.load('X_force_applied_tree%s_env%s.npy'%(tree, env)))
        y_vert_arrays.append(np.load('Y_vertex_final_pos_tree%s_env%s.npy'%(tree, env)))
    ...
    if PER_TREE:
        ...
        print(np.shape(y_vert_save))
        print(np.shape(force_applied_save))
        save(PATH + 'X_vertex_init_pose_tree%s'%(tree_num), x_vert_save)
        save(PATH + 'X_coeff_stiff_damp_tree%s'%(tree_num), coeff_save )
        save(PATH + 'X_edge_def_tree%s'%(tree_num), edge_def_save )
        save(PATH + 'X_force_applied_tree%s'%(tree_num), force_applied_save )
        save(PATH + 'Y_vertex_final_pos_tree%s'%(tree_num), y_vert_save)
...
if not PER_TREE:
    ...
    for idx in range(1,len(x_vert_save)):
        ...
        save(PATH + 'X_vertex_init_pose', x_vert_save)
        save(PATH + 'X_coeff_stiff_damp', coeff_save )
        save(PATH + 'X_edge_def', edge_def_save )
        save(PATH + 'X_force_applied', force_applied_save )
        save(PATH + 'Y_vertex_final_pos', y_vert_save)
