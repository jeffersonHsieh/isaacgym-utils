...
for tree in range(0, TREE_NUM):
    ...
    for init_vertex in init_vertices[1:]:
        init_vertex_variation += np.sum(np.absolute(first_init_vertex[0:3,:] - init_vertex[0:3,:]))
    ...
    displacement = 0
    max_displacement = max(np.sum(np.sum(np.absolute(init_vertices[:,0:3] - final_vertices[:,0:3]), axis=1), axis=1))
    min_displacement = min(np.sum(np.sum(np.absolute(init_vertices[:,0:3] - final_vertices[:,0:3]), axis=1), axis=1))
    ...
    displacement = np.sum(np.absolute(init_vertices[:,0:3] - final_vertices[:,0:3]))
...
#	        - sometimes displacement spikes heavily. Might lead to bad datapoints. Think about pruning datapoints where displacement is too high.
